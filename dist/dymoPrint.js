...
(function () {
  ...
  var setTextContent = function setTextContent(element, text) {
    ...
    } else if (element.firstChild && element.firstChild.nodeType == NodeType.TEXT) {
  ...
  Settings.prototype.load = function () {
    ...
    if (currentPrinterName) this.currentPrinterName = currentPrinterName;

    if (printerUris) this.printerUris = printerUris.split("|");
  };
  ...
  function onload() {
    ...
    function updatePrinters() {
      ...
      for (var i = 0; i < printers.length; i++) {
        ...
        if (printerName == settings.currentPrinterName) printersComboBox.selectedIndex = i;
      }
    ...
    function loadSettings() {
      ...
      for (var i = 0; i < settings.printerUris.length; ++i) {
        ...
        dymo.label.framework.addPrinterUri(printerUri, "", updatePrinters, function () {
    ...
    printerSettingsButton.onclick = function () {
      if (printerSettingsDiv.style.display == "none") printerSettingsDiv.style.display = "block";else printerSettingsDiv.style.display = "none";
    };
    ...
    printButton.onclick = function () {
      try {
        ...
        if (!printer) throw new Error("Select printer");
        ...
        if (!label) throw new Error("Label is not loaded. Wait until is loaded or reload the page");
    ...
    addPrinterUriButton.onclick = function () {
      try {
        ...
        if (!printerUri) throw new Error("Specify printer Url");

        dymo.label.framework.addPrinterUri(printerUri, "", function () {
  ...
  if (window.addEventListener) window.addEventListener("load", initTests, false);else if (window.attachEvent) window.attachEvent("onload", initTests);else window.onload = initTests;
})();